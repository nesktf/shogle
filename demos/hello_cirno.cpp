...
int main() {
  ...
  auto fumo =
    shogle::load_model<shogle::pnt_vertex>("./demos/res/cirno_fumo/cirno_fumo.obj", fumo_flag);
  if (!fumo) {
  ...
  auto sdf_rule = shogle::sdf_text_rule::create(ctx, shogle::color3{1.f, 0.f, 0.f}, 0.5f, 0.05f,
                                                shogle::color3{0.f, 0.f, 0.f},
                                                shogle::vec2{-0.005f, -0.005f}, 0.62f, 0.05f);
  if (!sdf_rule) {
  ...
  auto tex = shogle::make_texture2d(ctx, *cirno_img, shogle::texture_sampler::nearest,
                                    shogle::texture_addressing::repeat)
               .value();
  auto atlas_tex = shogle::make_texture2d(ctx, *atlas_img, shogle::texture_sampler::nearest,
                                          shogle::texture_addressing::repeat)
                     .value();
  auto fumo_tex = shogle::make_texture2d(ctx, *fumo_diffuse, shogle::texture_sampler::linear,
                                         shogle::texture_addressing::repeat)
                    .value();
  ...
  auto [fbo, fbo_tex] =
    shogle::make_fbo(ctx, {1280, 720}, {1.f, 0.f, 0.f, 1.f}, shogle::clear_flag::color_depth)
      .value();
  ...
  auto transf_cube = shogle::transform3d<ntf::float32>{}.pos(0.f, 0.f, 0.f).scale(1.f);
  ...
  auto transf_quad0 =
    shogle::transform2d<ntf::float32>{}.pos(-350.f, 0.f).scale(shogle::vec2{300.f, 300.f});
  auto transf_quad1 = shogle::transform2d<ntf::float32>{}
                        .pos(350.f, 0.f)
                        .scale(shogle::vec2{fb_ratio * 300.f, 300.f});
  ...
  auto transf_fumo = shogle::transform3d<ntf::float32>{}.pos(0.f, -.3f, 0.f).scale(fumo_scale);
  ...
  auto transf_rin =
    shogle::transform2d<ntf::float32>{}.pos(0.f, -200.f).scale(rin_scale * rin_aspect, -rin_scale);
  ...
  auto loop = ntf::overload{
    [&](ntf::uint32 ups) {
      ...
      transf_cube.rot(t, shogle::vec3{0.f, 1.f, 0.f})
        .scale_y(.5f + std::abs(std::sin(t)))
        .offset_y(-0.5f);
      transf_quad0.roll(-t);
      transf_quad1.roll(t);
      transf_fumo.rot(glm::pi<ntf::float32>() * .5f * t, shogle::vec3{0.f, 1.f, 0.f})
        .scale_y(fumo_scale * .5f +
                 fumo_scale * .5f * std::abs(std::sin(glm::pi<ntf::float32>() * t)));
    },
  ...
  shogle::render_loop(window, ctx, ups, loop);
