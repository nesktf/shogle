...
void text_buffer::_append_glyph(const glyph_metrics& glyph, float x, float y, float scale,
  ...
  const vec2 uv_scale{gsize_x / ((flip_x * -1.f + !flip_x * 1.f) * atlas_w),
                      gsize_y / ((flip_y * -1.f + !flip_y * 1.f) * atlas_h)};
  const vec2 uv_offset{(static_cast<float>(glyph.offset.x) + (flip_x * gsize_x)) / atlas_w,
...
sdf_text_rule::sdf_text_rule(pipeline&& pip, uniform_buffer&& ubo, u32 u_sampler, u32 u_transf,
                             const glyph_props& props) :
    _pipeline{std::move(pip)}, _uniform_buffer{std::move(ubo)}, _u_sampler{u_sampler},
    _u_transf{u_transf}, _props{props} {}

render_expect<sdf_text_rule> sdf_text_rule::create(context_view ctx, const color3& color,
                                                   float width, float edge,
                                                   const color3& outline_color,
                                                   const vec2& outline_offset, float outline_width,
                                                   float outline_edge) {
...
bitmap_text_rule::bitmap_text_rule(pipeline&& pip, uniform_buffer&& ubo, u32 u_sampler,
                                   u32 u_transf, const color3& color) :
    _pipeline{std::move(pip)}, _uniform_buffer{std::move(ubo)}, _u_sampler{u_sampler},
    _u_transf{u_transf}, _text_color{color} {}
...
font_renderer::font_renderer(shader_storage_buffer&& ssbo, texture2d&& atlas, font_glyphs&& glyphs,
                             glyph_map&& map, const mat4& transform, vec2 bitmap_extent,
                             size_t batch) noexcept :
    _ssbo{std::move(ssbo)}, _atlas_tex{std::move(atlas)}, _glyphs{std::move(glyphs)},
    _glyph_map{std::move(map)}, _transform{transform}, _bitmap_extent{bitmap_extent},
    _batch_sz{batch} {}

render_expect<font_renderer> font_renderer::create(context_view ctx, const mat4& transform,
                                                   font_atlas_data&& font, texture_sampler sampler,
                                                   size_t batch_size) {
  ...
  return font_renderer{std::move(*ssbo),    std::move(*tex), std::move(font.glyphs),
                       std::move(font.map), transform,       static_cast<vec2>(font.bitmap_extent),
                       batch_size};
}
