...
public:
  ...
  inline sprite_shader& bind_texture(const gl::texture& tex) {
    tex.bind(_sprite_sampler);
    set_uniform(_texture_unif, (int)_sprite_sampler);
    return *this;
  }

  inline sprite_shader& set_linear_offset(vec2 offset) {
    set_uniform(_offset_linear_unif, offset);
    return *this;
  }

  inline sprite_shader& set_const_offset(vec2 offset) {
