...
gl::mesh::mesh(mesh_primitive primitive, T* vertices, size_t vert_sz,
  ...
  glBufferData(GL_ARRAY_BUFFER, vert_sz, vertices, gl::enumtogl(vert_buff));
...
template<typename T, is_shader_attribute... attribs>
gl::mesh::mesh(mesh_primitive primitive, T* vertices, size_t vert_sz, mesh_buffer vert_buff,
  ...
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, ind_sz, indices, gl::enumtogl(ind_buff));
...
inline gl::mesh::~mesh() noexcept {
#ifdef SHOGLE_GL_RAII_UNLOAD
  unload();
#endif
}

inline gl::mesh::mesh(mesh&& m) noexcept :
...
inline auto gl::mesh::operator=(mesh&& m) noexcept -> mesh& {
  unload();
...
void gl::mesh::setup_vertex_attrib(curr_attrib, attribs... attrib) {
...
inline bool gl::mesh::draw() const {
  if (!valid()) {
    return false;
  }

  glBindVertexArray(_vao);
  const auto prim = gl::enumtogl(primitive());
  if (has_indices()) {
