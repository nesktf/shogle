...
enum class gl_builtin_type : uint32 {
  none = GL_NONE,
  s8  = GL_BYTE,
  u8  = GL_UNSIGNED_BYTE,
  s16 = GL_SHORT,
...
static inline gl_builtin_type gl_attrib_type(r_attrib_type type) {
  switch (type) {
    ...
    case r_attrib_type::mat4:  return gl_builtin_type::f32;
...
static constexpr GLint enumtogl(r_texture_format format) {
  switch (format) {
...
static constexpr GLint enumtogl(r_texture_sampler sampler) {
  switch (sampler) {
    case r_texture_sampler::linear:   return GL_LINEAR;
    case r_texture_sampler::nearest:  return GL_NEAREST;
    default:  return 0;
  }
  // TODO: Sampler for mipmaps
  // switch (filter) {
