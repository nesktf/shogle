...
uint32 gl_texture_type(r_texture_type type, uint32 count) {
  switch (type) {
    ...
    case r_texture_type::none:      return GL_NONE;
  };
  NTF_UNREACHABLE();
}
...
uint32 gl_texture_format(r_texture_format format) {
  switch (format) {
    ...
    case r_texture_format::none:    return GL_NONE;
  };
  NTF_UNREACHABLE();
}
...
uint32 gl_texture_sampler(r_texture_sampler sampler, bool mipmaps,
  ...
  switch (sampler) {
    case r_texture_sampler::linear: {
      if (mipmaps) {
        if (lin_level) {
          return lin_levels ? GL_LINEAR_MIPMAP_LINEAR : GL_LINEAR_MIPMAP_NEAREST;
        } else {
          return lin_level ? GL_NEAREST_MIPMAP_LINEAR : GL_NEAREST_MIPMAP_NEAREST;
        }
      } else {
        return GL_LINEAR;
      }
    ...
    case r_texture_sampler::none:     return GL_NONE;
  }
  NTF_UNREACHABLE();
}
...
uint32 gl_texture_address(r_texture_address address) {
  switch (address) {
    ...
    case r_texture_address::none:                 return GL_NONE;
  };
  NTF_UNREACHABLE();
};

bool gl_texture::load(const uint8** data, uint32 count, uint32 mipmaps, ivec3 dim,
  ...
  const uint32 glsampler = gl_texture_sampler(sampler, false, false, false);
