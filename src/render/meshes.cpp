...
quad_mesh quad_mesh::create(unchecked_t u, r_context_view ctx,
                            bool inverted_uvs, bool dynamic_storage) {
  r_buffer_flag flags = dynamic_storage ? r_buffer_flag::dynamic_storage : r_buffer_flag::none;
  auto ebo = renderer_buffer::create(u, ctx,
                                     pnt_indexed_quad_ind,
                                     r_buffer_type::index,
                                     flags);
  if (inverted_uvs) {
    auto vbo = renderer_buffer::create(u, ctx,
                                       pnt_indexed_quad_vert_inv,
                                       r_buffer_type::vertex,
                                       flags);
    return {std::move(vbo), std::move(ebo), inverted_uvs};
  } else {
    auto vbo = renderer_buffer::create(u, ctx,
                                       pnt_indexed_quad_vert,
                                       r_buffer_type::vertex,
...
cube_mesh cube_mesh::create(unchecked_t u, r_context_view ctx, bool dynamic_storage) {
  r_buffer_flag flags = dynamic_storage ? r_buffer_flag::dynamic_storage : r_buffer_flag::none;
  auto vbo = renderer_buffer::create(u, ctx,
                                     pnt_unindexed_cube_vert,
                                     r_buffer_type::vertex,
