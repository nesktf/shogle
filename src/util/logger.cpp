...
void logger::_do_log(log_level level, std::string_view prefix, const std::string& str) {
  static constexpr std::string_view level_pfxs[]{
    "ERROR", "WARNING", "INFO", "DEBUG", "VERBOSE",
  };
  static constexpr std::string_view level_colors[]{
...
[[noreturn]] void assert_failure(const char* cond, const char* func, const char* file, int line,
                                 const char* msg) {
  if (msg) {
    shogle::logger::log_error("CORE", "{}:{}: {} assertion '{}' failed: {}", file, line, func,
                              cond, msg);
  } else {
    shogle::logger::log_error("CORE", "{}:{}: {} assertion '{}' failed", file, line, func, cond,
                              msg);
  }
  SHOGLE_ABORT();
