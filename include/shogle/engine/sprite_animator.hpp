...
public:
  ...
  texture_animator(const AtlasPtr atlas, atlas_sequence first_sequence) : _atlas(atlas) {
    enqueue_sequence(first_sequence, 0);
  }
...
public:
  void enqueue_sequence(atlas_sequence sequence, uint loops) {
    ...
    _sequences.push(
      entry{.sequence = sequence, .duration = loops * static_cast<uint>(seq.size())});
  }
