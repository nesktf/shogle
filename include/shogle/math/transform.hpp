...
// TODO: Rewrite this mess and make something actually useful

#define SHOGLE_TRANSF_DEF_SETTER(_signature, ...)                                         \
  Derived & _signature& noexcept(noexcept(static_cast<Derived&>(*this)._mark_dirty())) {  \
    __VA_ARGS__                                                                           \
...
template<typename T, typename Derived, u32 dim>
class transform_dim;
...
public:
  ...
  SHOGLE_TRANSF_DEF_SETTER(pos(const T& x, const complex<T>& pos_yz), _pos.x = x;
                           _pos.y = pos_yz.real(); _pos.z = pos_yz.imag(););
  ...
  SHOGLE_TRANSF_DEF_SETTER(scale(const complex<T>& scale_xy, const T& z),
                           ...
                           _scale.y = scale_xy.imag(); _scale.z = z;);
  SHOGLE_TRANSF_DEF_SETTER(scale(const T& x, const complex<T>& scale_yz), _scale.x = x;
                           _scale.y = scale_yz.real(); _scale.z = scale_yz.imag(););
  SHOGLE_TRANSF_DEF_SETTER(scale(const T& scale), _scale.x = scale; _scale.y = scale;
                           _scale.z = scale;);
  ...
  SHOGLE_TRANSF_DEF_SETTER(pivot(const complex<T>& pivot_xy, const T& z),
                           ...
                           _pivot.y = pivot_xy.imag(); _pivot.z = z;);
  SHOGLE_TRANSF_DEF_SETTER(pivot(const T& x, const complex<T>& pivot_yz), _pivot.x = x;
                           _pivot.y = pivot_yz.real(); _pivot.z = pivot_yz.imag(););
  ...
  SHOGLE_TRANSF_DEF_SETTER(offset(const complex<T>& offset_xy, const T& z),
                           ...
                           _offset.y = offset_xy.imag(); _offset.z = z;);
  SHOGLE_TRANSF_DEF_SETTER(offset(const T& x, const complex<T>& offset_yz), _offset.x = x;
                           _offset.y = offset_yz.real(); _offset.z = offset_yz.imag(););
  SHOGLE_TRANSF_DEF_SETTER(offset_x(const T& x), _offset.x = x;);
...
public:
  ...
  SHOGLE_TRANSF_DEF_SETTER(rot(const T& x, const complex<T>& rot_yz), _rot.x = x;
                           _rot.y = rot_yz.real(); _rot.z = rot_yz.imag(););
  SHOGLE_TRANSF_DEF_SETTER(pitch(const T& pitch), _rot.x = pitch;);
  SHOGLE_TRANSF_DEF_SETTER(yaw(const T& yaw), _rot.y = yaw;);
...
public:
  ...
  SHOGLE_TRANSF_DEF_SETTER(rot_w(const T& w), _rot.w = w;);
...
template<typename T, typename Transform, u32 dim, bool use_euler>
...
public:
  ...
  static constexpr u32 dimensions = dim;
...
public:
  explicit basic_transform(
    ...
    const Transform& transf = {}) noexcept(std::is_nothrow_copy_constructible_v<Transform> &&
      ...
      _local_mat{T{1}}, _gen_mat{transf}, _flags{flags} {
    _mark_dirty();
  }
...
public:
  ...
  [[nodiscard]] const mat<4, 4, T>& local() & noexcept(noexcept(force_update())) {
  ...
  void force_update() & noexcept(noexcept(_gen_mat(this->_pos, this->_scale, this->_pivot,
                                                   this->_offset, this->_rot))) {
