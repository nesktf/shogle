...
void destroy_shader(shader& shad);
void destroy_texture(texture& tex);
void destroy_mesh(mesh& mesh);
void destroy_framebuffer(framebuffer& fbo);

void set_tex_filter(texture& tex, res::texture_filter filter);

void draw_mesh(const mesh& mesh);
void draw_quad(const texture& texture, bool inverted = false);

inline void framebuffer_bind(const framebuffer* fbo) {
  if (fbo) {
...
inline void texture_bind(const texture& tex, size_t number = 0) {
  glBindTexture(tex.type, tex.id);
  glActiveTexture(GL_TEXTURE0+number);
}

inline void clear_viewport(vec4 col = {vec3{0.2f}, 1.0f}, bool depth = true, bool stencil = false) {
...
inline void depth_test(bool flag = true) {
  if (flag) {
...
inline void stencil_test(bool flag = true) {
  if (flag) {
...
inline void blend(bool flag = true) {
  if (flag) {
    glEnable(GL_BLEND);
...
inline void shader_enable(shader& shader) {
  glUseProgram(shader.prog);
}
inline void shader_unif(shader& shader, const char* name, long unsigned int val) {
